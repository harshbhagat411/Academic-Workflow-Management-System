  ...
  const filteredData = React.useMemo(() => {
    return data.filter((user) => {
      ...
      const matchesSearch =
        ...
        (user.loginId?.toLowerCase() || "").includes(
          searchTerm.toLowerCase(),
        ) ||
        (user.department?.toLowerCase() || "").includes(
          searchTerm.toLowerCase(),
        ) ||
        (user.specialization?.toLowerCase() || "").includes(
          searchTerm.toLowerCase(),
        );
            ...
            {showLoading ? (
              ...
              filteredData.map((user, index) => (
                <TableRow hover key={user._id}>
                  ...
                  {type === "Student" && (
                    <TableCell>Sem {user.semester}</TableCell>
                  )}
                  {type === "Student" && (
                  ...
                  <TableCell align="right">
                    <MuiButton
                      variant={
                        user.status === "Active" ? "outlined" : "contained"
                      }
...
const AdminDashboard = () => {
  ...
  const [bulkLectures, setBulkLectures] = useState([
    { startTime: "", endTime: "", subjectId: "", facultyId: "" },
  ]);
  ...
  const fetchSections = async (semester) => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:5000/api/sections?semester=${semester}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      ...
      if (res.data.length > 0 && !timetableFilter.section) {
        console.log(
          `[AdminDashboard] Auto-selecting section: ${res.data[0].sectionName}`,
        );
        setTimetableFilter((prev) => ({
          ...prev,
          section: res.data[0].sectionName,
        }));
      }
    } catch (err) {
  ...
  const handleCreateSection = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        "http://localhost:5000/api/sections/create",
        createSectionData,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleAssignStudents = async () => {
    try {
      ...
      const res = await axios.post(
        "http://localhost:5000/api/sections/assign",
        assignData,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  useEffect(() => {
    ...
    activeTab,
    timetableFilter.semester,
    timetableFilter.section,
    semesterFilter,
  ]); // Refetch when semester/section changes
  ...
  const fetchTimetable = async () => {
    try {
      const token = localStorage.getItem("token");
      console.log(
        `[AdminDashboard] Fetching timetable for Sem: ${timetableFilter.semester}, Section: ${timetableFilter.section}`,
      );
      const res = await axios.get(
        `http://localhost:5000/api/timetable?semester=${timetableFilter.semester}&section=${timetableFilter.section}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const handleAddLecture = async (e) => {
    ...
    try {
      ...
      if (!facultyId) {
        alert(
          "Please select a subject with an assigned faculty or select a faculty manually.",
        );
        return;
      }
  ...
  const handleCopyTimetable = async () => {
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        "http://localhost:5000/api/timetable/copy",
        {
  ...
  const handleBulkSave = async () => {
    try {
      ...
      const validLectures = bulkLectures.filter(
        (l) => l.startTime && l.endTime && l.subjectId,
      );
      ...
      await axios.post(
        "http://localhost:5000/api/timetable/bulk-add",
        {
  ...
  const handleCreateSubject = async (e) => {
    ...
    try {
      ...
      const subjectData = {
        ...newSubject,
        department: newSubject.department || "Computer Science",
      };
  ...
  const handleAssignCounselor = async () => {
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        "http://localhost:5000/api/mentors/assign",
        counselorForm,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  return (
    <Layout role="Admin" activeTab={activeTab} setActiveTab={setActiveTab}>
        ...
        <Box sx={{ mt: 3 }}>
          ...
          >
            <CardContent>
              ...
              >
                System is fully operational. Use the navigation tabs or the
                quick actions below to manage the platform.
              </Typography>
            ...
            >
                    ...
                    >
                      <input
                        ...
                        checked={
                          ...
                          counselorForm.studentIds.length ===
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                ...
                <MuiButton
                  ...
                  disabled={
                    counselorForm.studentIds.length === 0 ||
                    !counselorForm.facultyId
                  }
                  sx={{ mt: "auto", py: 1.5 }}
                >
                  Assign Counselor
                ...
                <TableBody>
                  {counselorAllocations.length === 0 ? (
                    <TableRow>
                      ...
                      >
                        No allocations found. Start by assigning a counselor
                        above.
                      </TableCell>
                    </TableRow>
                  ) : (
            ...
            >
              ...
              <FormControl fullWidth size="small" required>
                ...
                <MuiSelect
                  ...
                  onChange={(e) =>
                    setNewSubject({ ...newSubject, facultyId: e.target.value })
                  }
                >
                  <MenuItem value="">
                    <em>Select Faculty</em>
          ...
          >
            ...
            >
              ...
              >
                <InputLabel>Semester</InputLabel>
                <MuiSelect
                  value={timetableFilter.semester}
                  label="Semester"
                  onChange={(e) =>
                    setTimetableFilter({
                      ...timetableFilter,
              ...
              >
                ...
                >
                  {sections.length > 0 ? (
                    sections.map((s) => (
                      <MenuItem key={s._id} value={s.sectionName}>
            ...
            >
              ...
              >
                ...
                <FormControl fullWidth size="small" required>
                  ...
                  <MuiSelect
                    value={
                      newLecture.startTime && newLecture.endTime
                        ? `${newLecture.startTime}|${newLecture.endTime}`
                        : ""
                    }
                    label="Time Slot"
                  ...
                  >
                    ...
                    {(lectureType === "Lab" ? labSlots : theorySlots).map(
                      (slot, idx) => (
                        <MenuItem key={idx} value={getSlotValue(slot)}>
                          {slot}
                        </MenuItem>
                      ),
                    )}
                  ...
                  >
                    ...
                    {subjects
                      .filter(
                        (s) =>
                          s.semester.toString() ===
                          timetableFilter.semester.toString(),
                      )
                      .map((s) => (
          ...
          <div className="bg-slate-800/50 rounded-2xl shadow-xl border border-slate-700/50 overflow-hidden animate-fade-in-up delay-100 backdrop-blur-sm">
            <div className="p-6 border-b border-slate-700/50 bg-slate-900/30 flex justify-between items-center">
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                ...
                Weekly Timetable (Semester {timetableFilter.semester} - Section{" "}
                {timetableFilter.section})
              </h3>
              <span className="text-sm font-semibold text-slate-400 bg-slate-700/50 px-3 py-1 rounded-full">
                ...
                <thead>
                  <tr>
                    ...
                    {[
                      ...
                      <th
                        key={day}
                        className="p-4 bg-slate-900 text-white border-r border-slate-700 last:border-r-0 min-w-[160px]"
                      >
                ...
                <tbody className="divide-y divide-slate-700/50">
                  {ACADEMIC_SLOTS.map((slot, idx) => {
                    if (slot.type === "break") {
                      return (
                        <tr key={idx} className="bg-amber-900/20">
                          <td
                            colSpan="7"
                            className="p-3 text-center border-y-2 border-amber-900/30"
                          >
                    ...
                    return (
                      <tr
                        key={idx}
                        className="hover:bg-slate-800/30 transition-colors h-24"
                      >
                        {/* Time Column */}
                        ...
                        {[
                          ...
                          const lecture = timetable.find(
                            (t) => t.day === day && t.startTime === slot.start,
                          );
                          const isOccupied = timetable.some(
                            (t) =>
                              t.day === day &&
                              t.startTime < slot.start &&
                              t.endTime > slot.start,
                          );
                          ...
                          if (lecture) {
                            ...
                            const endHour = parseInt(
                              lecture.endTime.split(":")[0],
                            );
                            const endMin = parseInt(
                              lecture.endTime.split(":")[1],
                            );
                          ...
                          return (
                            <td
                              key={day}
                              rowSpan={rowSpan}
                              className={`p-4 border-r border-slate-700 relative group transition-all hover:bg-slate-800/50 vertical-top ${rowSpan > 1 ? "z-10" : ""}`}
                            >
                              {lecture ? (
                                <div
                                  className={`absolute inset-2 border rounded-lg p-2 flex flex-col justify-between group hover:shadow-md transition-all ${
                                    lecture.type === "Lab"
                                ...
                                >
                                  <div className="flex-1 flex flex-col justify-center">
                                    ...
                                    <div
                                      className={`text-[10px] font-mono uppercase rounded px-1 w-max mx-auto ${
                                        lecture.type === "Lab"
                                  ...
                                  <div className="mt-2 flex items-center gap-1.5 justify-center bg-slate-800/80 rounded-md py-1 px-2 border border-blue-800/30">
                                    ...
                                    <span className="text-[10px] text-slate-300 truncate max-w-[80px] font-medium">
                                      {lecture.facultyId?.name}
                                    </span>
                                  </div>
            ...
            <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 animate-fade-in backdrop-blur-sm">
              <div className="bg-slate-800 p-8 rounded-2xl shadow-2xl w-96 transform transition-all scale-100 border border-slate-700">
                ...
                <p className="text-slate-400 mb-4 text-sm">
                  Target: {timetableFilter.day} (Sem {timetableFilter.semester})
                </p>
                ...
                <div className="space-y-3 mb-6 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
                    Select Lectures ({selectedLectureIds.length} /{" "}
                    {timetable.filter((t) => t.day === copySourceDay).length})
                  </label>

                  {timetable.filter((t) => t.day === copySourceDay).length ===
                  0 ? (
                    <p className="text-sm text-slate-500 italic">
                      No lectures found on {copySourceDay}.
                    </p>
                  ) : (
                    timetable
                      ...
                      .map((t) => (
                        <div
                          key={t._id}
                          className="flex items-center gap-3 p-3 bg-slate-700/50 rounded-xl border border-slate-600 hover:bg-slate-600/50 transition-colors cursor-pointer"
                          onClick={() => {
                            if (selectedLectureIds.includes(t._id)) {
                              setSelectedLectureIds(
                                selectedLectureIds.filter((id) => id !== t._id),
                              );
                            } else {
                              setSelectedLectureIds([
                                ...selectedLectureIds,
                        ...
                        >
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-xs text-slate-400 font-mono">
                              {t.startTime} - {t.endTime} • {t.facultyId?.name}
                            </div>
                          </div>
                        </div>
                      ))
                ...
                <div className="flex justify-end gap-3">
                  ...
                  <button
                    ...
                    className="bg-purple-600 text-white px-6 py-2 rounded-lg hover:bg-purple-700 shadow-lg shadow-purple-500/30"
                  >
                    Copy
                  </button>
                </div>
              ...
              <div className="bg-slate-800 p-8 rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto border border-slate-700 custom-scrollbar">
                <div className="flex justify-between items-center mb-8 border-b border-slate-700 pb-4">
                  ...
                  >
                    ...
                    {/* Close icon visual using logout rotated, or simple X would be better but reusing imported icons */}
                  </button>
                </div>
                ...
                <div className="space-y-4 mb-8">
                  {bulkLectures.map((row, idx) => (
                    <div
                      key={idx}
                      className="group grid grid-cols-1 md:grid-cols-12 gap-6 items-end bg-slate-700/30 hover:bg-slate-700/50 p-6 rounded-2xl border border-slate-600 hover:border-indigo-400/50 hover:shadow-lg transition-all duration-300 relative"
                    >
                      ...
                      <div className="md:col-span-2 flex justify-end">
                        <button
                          onClick={() => {
                            const newRows = bulkLectures.filter(
                              (_, i) => i !== idx,
                            );
                            setBulkLectures(newRows);
                          }}
                ...
                <div className="flex justify-between items-center pt-6 border-t border-slate-700">
                  ...
                  <div className="flex gap-4">
                    <button
                      onClick={() => setShowBulkModal(false)}
                      className="px-6 py-3 text-slate-400 hover:bg-slate-700/50 hover:text-white rounded-xl font-bold transition-colors"
                    >
                      Cancel
                    </button>
                    <button
                      onClick={handleBulkSave}
                      className="bg-indigo-600 text-white px-8 py-3 rounded-xl hover:bg-indigo-700 shadow-xl shadow-indigo-500/30 font-bold transition-transform hover:-translate-y-0.5 active:scale-95 flex items-center gap-2"
                    >
                      <Upload size={20} />
                      Save All Lectures
                    </button>
          ...
          <Grid container spacing={3}>
            {sections.length === 0 ? (
              <Grid size={{ xs: 12 }}>
                ...
                >
                  {semesterFilter
                    ? "No sections found. Sections are auto-created when students are added."
                    : "Select a semester to view sections."}
                </Typography>
              </Grid>
            ) : (
                    ...
                    <CardContent>
                      ...
                      >
                        ...
                        <Chip
                          ...
                          size="small"
                          color={
                            section.status === "Active" ? "success" : "error"
                          }
                          variant="filled"
                        />
                      </Box>
                        ...
                        >
                          <Box
                            sx={{
                              ...
                              height: "100%",
                              transition: "all 0.3s",
                              width: `${Math.min((section.currentStrength / section.maxCapacity) * 100, 100)}%`,
                            }}
                          />
                        </Box>
                      </Box>
                      ...
                      >
                        <MuiButton
                          ...
                          variant="contained"
                          color={
                            section.status === "Active" ? "inherit" : "primary"
                          }
                          sx={{
                            bgcolor:
                            ...
                            try {
                              ...
                              const newStatus =
                                section.status === "Active"
                                  ...
                                  : "Active";
                              await axios.patch(
                                `http://localhost:5000/api/sections/${section._id}`,
                                { status: newStatus },
                                {
                                  headers: { Authorization: `Bearer ${token}` },
                                },
                              ...
                              const res = await axios.get(
                                "http://localhost:5000/api/sections?semester=" +
                                  (semesterFilter || section.semester),
                                {
                                  headers: { Authorization: `Bearer ${token}` },
                                },
                              );
          ...
          >
            ...
            >
              ...
              >
                ...
                <IconButton onClick={() => setShowAssignModal(false)}>
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <MuiButton
                        ...
                        onClick={() => {
                          if (assignData.studentIds.length > 0) {
                            ...
                            const toSelect = unassignedStudents
                              .slice(0, available)
                              .map((s) => s._id);
                            setAssignData({
                              ...assignData,
                      ...
                      >
                        {assignData.studentIds.length > 0
                          ? "Deselect All"
                          : "Select Max Fit"}
                      </MuiButton>
                    </Box>
                        ...
                        unassignedStudents.map((student) => (
                          ...
                          >
                            <input
                              type="checkbox"
                              style={{ width: 16, height: 16 }}
                              checked={assignData.studentIds.includes(
                                student._id,
                              )}
                              onChange={(e) => {
                                if (e.target.checked) {
                    ...
                    <Typography
                      ...
                      sx={{ pt: 1 }}
                    >
                      Selected: {assignData.studentIds.length}
                    </Typography>
                  </Paper>
                </Grid>
                  ...
                  <Box>
                    ...
                    <FormControl fullWidth>
                      ...
                      >
                        ...
                        {sections.map((s) => (
                          <MenuItem key={s._id} value={s._id}>
                            Section {s.sectionName} ({s.currentStrength}/
                            {s.maxCapacity})
                          </MenuItem>
                        ))}
                      </MuiSelect>
                    ...
                    >
                      Ensure the section has enough capacity for the selected
                      students.
                    </Typography>
                  </Box>
      ...
      <ConfirmDialog
        ...
        confirmText={
          confirmDialog.type === "danger" ? "Yes, Continue" : "Confirm"
        }
        onConfirm={() => {
